...
namespace LibMPlayerCommon
{
    ...
    public class Discover
    {
        ...
        public Discover(string filePath)
        {
            ...
            //Flags whether the next line is going to be the 'Title' metadata
            bool isNextLineTitle = false;
            ...
            //while (handle.HasExited == false)
            {
                ...
                //if the previous line told us that we're waiting for the file's Name metadata... this is it!
                if (true == isNextLineTitle)
                {
                    int equalsPosition = line.IndexOf("=");
                    if (equalsPosition == -1)
                    {
                        throw new Exception("Error - was expecting the 'Title' metadata, but instead got: " + line);
                    }
                    _Title = line.Substring(equalsPosition + 1);
                    //reset the flag
                ...
                if (line.StartsWith("ID_CLIP_INFO_NAME") && line.EndsWith("=Title"))
                {
