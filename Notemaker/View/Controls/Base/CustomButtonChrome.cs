...
{
    /// <summary>
    /// Custom button chrome, to get rid of the white border when background is set to transparent.
    /// From guy in answers section: http://stackoverflow.com/questions/4553692/how-to-remove-buttonchrome-border-when-defining-the-template-of-a-border
    /// Made two tweaks:-
    ///  - disabling the border when the button is disabled
    ///  - removing background (making it transparent) when button is disabled
    /// </summary>
    public sealed class CustomButtonChrome : Decorator
    {
        private static Pen _transparentPen = new Pen(Brushes.Transparent, 1);
        ...
        public static readonly DependencyProperty BackgroundProperty = Control.BackgroundProperty.AddOwner(typeof(CustomButtonChrome), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.AffectsRender));
        public static readonly DependencyProperty BorderBrushProperty = Border.BorderBrushProperty.AddOwner(typeof(CustomButtonChrome), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.AffectsRender));
        public static readonly DependencyProperty RenderDefaultedProperty = DependencyProperty.Register("RenderDefaulted", typeof(bool), typeof(CustomButtonChrome), new FrameworkPropertyMetadata(false, new PropertyChangedCallback(CustomButtonChrome.OnRenderDefaultedChanged)));
        public static readonly DependencyProperty RenderMouseOverProperty = DependencyProperty.Register("RenderMouseOver", typeof(bool), typeof(CustomButtonChrome), new FrameworkPropertyMetadata(false, new PropertyChangedCallback(CustomButtonChrome.OnRenderMouseOverChanged)));
        public static readonly DependencyProperty RenderPressedProperty = DependencyProperty.Register("RenderPressed", typeof(bool), typeof(CustomButtonChrome), new FrameworkPropertyMetadata(false, new PropertyChangedCallback(CustomButtonChrome.OnRenderPressedChanged)));
        public static readonly DependencyProperty RoundCornersProperty = DependencyProperty.Register("RoundCorners", typeof(bool), typeof(CustomButtonChrome), new FrameworkPropertyMetadata(true, FrameworkPropertyMetadataOptions.AffectsRender));
        public static readonly DependencyProperty DisableInnerBorderProperty = DependencyProperty.Register("DisableInnerBorder", typeof(bool), typeof(CustomButtonChrome), new FrameworkPropertyMetadata(true, FrameworkPropertyMetadataOptions.AffectsRender));

        static CustomButtonChrome()
        {
            UIElement.IsEnabledProperty.OverrideMetadata(typeof(CustomButtonChrome), new FrameworkPropertyMetadata(true, FrameworkPropertyMetadataOptions.AffectsRender));
        }
        ...
        private void DrawBorder(DrawingContext dc, ref Rect bounds)
        {
            if ((bounds.Width >= 5.0) && (bounds.Height >= 5.0))
            {
                ...
                {
                    //JDW: only draw the border if the control is enabled... otherwise we get awful grey borders around the buttons when disabled.
                    if (this.IsEnabled)
                    {
        ...
        {
            CustomButtonChrome chrome = (CustomButtonChrome)o;
            if (chrome.Animates)
            {
            ...
            get
            {
                if (_commonDisabledBackgroundOverlay == null)
                {
                    lock (_resourceAccess)
                    {
                        if (_commonDisabledBackgroundOverlay == null)
                        {
                            //JDW: don't return ANY background overlay. return transparent.

                            //SolidColorBrush brush = new SolidColorBrush(Color.FromRgb(0xf4, 0xf4, 0xf4));
                            //brush.Freeze();
                            //_commonDisabledBackgroundOverlay = brush;

                            _commonDisabledBackgroundOverlay = Brushes.Transparent;
                        }
                    }
                }
